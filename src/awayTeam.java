public class awayTeam {
	...
	//run a possession: 
		...
		int shotsSatisfied = 0; 
        while(shotsSatisfied != 1) {
            shotsSatisfied = 0; 
            if(shotType==0 && (awayPlayersOnCourt[shotTakerID].getftA()/2.2)/shotAttemptDenominator > Math.random()) {shotType = 1; shotsSatisfied += 1;}
            if(shotType==0 && (awayPlayersOnCourt[shotTakerID].getTwoPA())/shotAttemptDenominator > Math.random()) {shotType = 2; shotsSatisfied += 1;}
            if(shotType==0 && (awayPlayersOnCourt[shotTakerID].getThreePA())/shotAttemptDenominator > Math.random()) {shotType = 3; shotsSatisfied += 1;} 
        }
		...
		if(blockedShot == false) {
			...
			if(shotType==2) {
				...
				//account for home vs away 
				shotProbability -= 0.00525; 
