...
public class predictionClass {
	...
	public static void main(String[] args) {
        ...
        if(response.equals("y")) {
            ...
            ArrayList<Integer> injuredPlayers = new ArrayList<Integer>(); 
            while(!response.equals("$")) {
                response = myObj.nextLine(); 
                if(!response.equals("$")) {
                    int injuredPlayer = Integer.valueOf(response); 
                    injuredPlayers.add(injuredPlayer); 
                }
            }
            for(int x : injuredPlayers) {
            ...
            for(int x : injuredPlayers) {
                awayTeam.totalAwayPlayers[x].setAvgMins(0.0); 
	...
	public static String game(String team_home, String team_away) {
		...
		double gamePace = (homePace+awayPace)/2; 
		int homeScore = 0; 
		int awayScore = 0; 
		for(int i=0; i<gamePace; i++) {
